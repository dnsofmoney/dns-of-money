...
MemoData:   hex(JSON string)
```

The Memo JSON is minified before hex encoding. The `n` field (sequence number) is assigned atomically at INSERT time and represents the registrant's position in the founding cohort.
...
FAS-1 uses XRPL Payment transactions with structured Memo fields as immutable on-chain proofs of registration. This is not NFT minting (Phase 1). NFT identity anchoring is a Phase 2 feature.
...
Memos: [
  {
    Memo: {
      MemoType:   "4641532D31",         ← "FAS-1"
      MemoFormat: "6170706C...",        ← "application/json"
      MemoData:   "7B2270223A..."       ← minified JSON
    }
  }
]
```
...
3. Confirming `p === "FAS-1"`, `a === alias`, `w === registrant_address`
...
The `iso20022_hint` field in the resolution response provides structured metadata for ISO 20022 message construction. These are hints — not fully-formed messages. Message construction is the responsibility of the payment orchestration layer (Layer 2 of the Financial Autonomy Stack).
...
This allows any MCP-compatible AI agent (Claude, GPT, Gemini, open-source) to resolve payment addresses natively within their tool call loop.
...
Resolution requests MUST be screened against OFAC sanctions lists. A positive hit returns `COMPLIANCE_BLOCKED` (403). Screening results are cached for 24 hours maximum. Fresh screening is required after 24h.
...
| v0.2 | Draft | AP2 section, founding tier fields, compliance signals |
| v1.0 | Planned | Stable release, NFT identity anchoring, sub-namespace delegation, MCP tool surface |

Breaking changes to the URI syntax, resolution response schema, or XRPL memo schema require a version increment. Additive fields (new optional response fields) are non-breaking.

---

## 12. Reference Implementation

The FAS-1 reference implementation is the Financial Autonomy Stack (ucap), available at:

- **Private (full stack):** https://github.com/sparkmastergrape/ucap
- **Public spec repo:** https://github.com/dnsofmoney/dns-of-money
- **Public protocol core:** https://github.com/dnsofmoney/a2a-protocol-core

The reference implementation runs on:
- Python 3.11+, FastAPI, PostgreSQL 15+, SQLAlchemy 2, Redis 7
- EC2 with Nginx reverse proxy
- XRPL mainnet (production anchor) and XRPL testnet (development)

---

## Appendix A: Reserved Namespace Registry

The following namespaces are pre-registered and controlled by DNS of Money. Registration attempts against these names return `ALIAS_RESERVED`.
...
*FAS-1 v0.2 Draft — DNS of Money Project — Financial orchestration infrastructure for the machine economy. We are not a bank. We are the layer above it.*
